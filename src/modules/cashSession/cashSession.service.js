// src/modules/cashSession/cashSession.service.js
const sequelize = require('../../config/database');
const cashSessionRepository = require('./cashSession.repository');
const CashRegister = require('../../models/mysql/cashRegister.model');
const CashMovement = require('../../models/mysql/cashMovement.model'); // ✅ agregado
...
class CashSessionService {
    ...
    async openSession(data, currentUser, req) {
        ...
        try {
            ...
            const existing = await cashSessionRepository.findOpenByRegister(
                cash_register_id,
            ...
            // 🔸 Actualizar caja con el id de la sesión abierta
            ...
            // 💰 Registrar movimiento de fondo inicial y marcar flag
            if (opening_balance && parseFloat(opening_balance) > 0) {
                ...
                // ✅ Marcar la sesión como con movimientos
                await session.update(
                    { has_movements: true },
                    { transaction: t }
                );
            } else {
                // 🧾 Apertura sin fondo → marcar explícitamente como sin movimientos
                await session.update(
                    { has_movements: false },
                    { transaction: t }
                );
            ...
            await createLog({
                ...
                description: `Apertura de caja ${cash_register_id} con fondo inicial de $${opening_balance || 0}`,
                ip: req.ip,
    ...
    async closeSession(id, data, currentUser, req) {
        ...
        try {
            ...
            // 🔹 Cerrar sesión
            await cashSessionRepository.closeSession(
                session,
                {
                    closed_at: new Date(),
            ...
            // 🔸 Liberar la caja (sin sesión activa)
            await CashRegister.update(
    ...
    async getDatatable(body, currentUser) {
        ...
        const orderColumnIndex =
            body['order[0][column]'] || (body.order?.[0]?.column ?? 0);
        const orderDir =
            (body['order[0][dir]'] || (body.order?.[0]?.dir ?? 'asc')).toUpperCase();
        ...
        const statusFilter =
            body['columns[2][search][value]'] || (body.columns?.[2]?.search?.value ?? '');
